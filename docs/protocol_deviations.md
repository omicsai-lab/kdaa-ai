...
| PC-09 | Ablation/robustness subset reuse | A1/A4/A6 and R1/R2/R3 run on the **same** already-frozen 60-case final LLM subset (30 stratified core + 30 challenge, 3-per-category) rather than a second, separately selected subset. No new selector is defined. Closes a gap surfaced when attempting final execution: PC-08 named which ablation/robustness families run in the final primary result package but never specified which cases they run on, and no `select_final_ablation_subset`/`select_final_robustness_subset` ever existed. Reuse minimizes additional degrees of freedom, preserves balanced core/challenge coverage, and permits direct paired comparison with C3/C4 on the same cases. Recorded before any final data exist -- a prospective clarification, not a post-result deviation. |

Full normative text: [`paper_b_protocol_clarifications_v1.0.1.md`](paper_b_protocol_clarifications_v1.0.1.md) for PC-01 through PC-05; PC-06 through PC-09 are defined in full in [`paper_b_final_evaluation_freeze.md`](paper_b_final_evaluation_freeze.md).
